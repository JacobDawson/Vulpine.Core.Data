...
namespace Vulpine.Core.Data.Trees
{
    ...
    /// it is able to garentee that any path from the root to a leaf is never more that
    /// twice as long as any sutch path. This dose not result in a perfect balancing
    /// of the tree, but insertion and removal are very effecent, and it offers
    /// ammortised O(log n) look-up time.
    /// </summary>
    ...
    where E : IComparable<E>
    {
        ...
        /// All nodes start out being collored black.
        /// </summary>
        ...
        {
            //colors the node red, and begins rebalancing
            SetRed(node, true);
            InsertCase1(node);
        }
        ...
        /// </summary>
        /// <param name="node">Node being procesed</param>
        private void InsertCase1(NodeBinary<E> node)
        {
            ...
            {
                //must continue if the parrent is red
                InsertCase2(node);
            }
        }
        ...
        /// grandparent recollored red.
        /// </summary>
        /// <param name="node">Node being procesed</param>
        private void InsertCase2(NodeBinary<E> node)
        {
            //determins the parrent and grandparent
            NodeBinary<E> parr = node.Parent;
            NodeBinary<E> grand = parr.Parent;
            NodeBinary<E> uncle = parr.GetSibling();

            //if the parrent and uncle are both red, repaint them black
            //repaint the grandparent red and treet as a new insertion
        ...
        /// is black. The parent is roatated so that the target becomes
        /// the new parent.
        /// </summary>
        /// <param name="node">Node being procesed</param>
        ...
        private void InsertCase4(NodeBinary<E> node)
        {
            ...
            //swaps the collors of parrent and grandparent
            SetRed(parr, false);
            SetRed(grand, true);
        ...
        /// is being deleted. It also enshures the sibling is black
        /// by proccess of rotation.
        /// </summary>
        /// <param name="node">Node being procesed</param>
        private void DeleteCase1(NodeBinary<E> node)
        {
            //no rotations nessary if new root
            ...
            if (IsRed(sibling))
            {
                //flips the parrent and slblings colors
                SetRed(parent, true);
        ...
        private void DeleteCase2(NodeBinary<E> node)
        {
            ...
            //determins if the children of S are black            
        ...
        /// the parent, so that DeleteCase4 will work corectly.
        /// </summary>
        /// <param name="node">Node being procesed</param>
        private void DeleteCase3(NodeBinary<E> node)
        {
            ...
            //the sibling cannot have two black clindren due to case2
        ...
        /// <summary>
        /// Helper method, used to determin if a given node is
        /// collored either red or black. Null nodes, are considered
        ...
        /// and set false to collor it black</param>
    ...
    {
        //determins if the given node is red
        private bool is_red;
        ...
        /// <param name="red">Set true to collor the node red,
        /// false to color it black</param>
        public NodeRedBlack(E item, bool red = false)
            : base(item) { is_red = red; }

        /// <summary>
        /// Determins the color of the node. It returns true if the
